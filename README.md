...
| | 示例 |
|-----|-----|
|参赛队伍名称（评测开始前可修改，只用于评测过程中的队伍区分，不用于论文写作）| 禁止蕉绿 |
|队伍联系人姓名（主要负责与组织方的邮件沟通，评测过程中不建议更换）| 丛冠良 |
|单位信息（评测结束前可修改，最终将用于成绩榜单以及评测综述论文中）| 澳门大学人文学院中国语言文学系 |
|计划参加赛道（评测结束前可修改）| 提示词赛道+微调赛道 |
...
- 队长不能作为队员参与其他队伍；
- 队伍成员人数不限;
- 参赛队伍名称评测开始前可修改，并且只用于评测过程中的队伍区分，不用于论文写作。
...
- 真值情况包括三种：
  - 如果模型认为，根据背景句的内容，结论句为真，则在"factivity"字段中写入“true”；同时在"confidence"字段中写入模型对该判定的置信度（即模型认为结论句在多大程度上为真），置信度的取值范围为(0.5, 1]，字段值类型为字符串（数值保留到小数点后两位）；
  ...
  - 如果模型认为，根据背景句的内容，不能判断结论句的真假，则在"factivity"字段中写入“uncertain”；同时在"confidence"字段中写入null，注意此处字段值类型即为空值（null）。
- 若模型拒绝回答，请调整提示词，重新测试。
- 若遇到其他问题，请邮件联系任务负责人。
