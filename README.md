...
## 4.2 评分方式：梯度匹配（Gradient Matching）

为了更准确地评估大模型的叙实性推理能力，本次评测采用**梯度匹配**（Gradient Matching）的方式计分，即：模型回答与标准回答（gold standard）所落入的叙实性强度区间越接近，得分越高。具体规则如下：

- 若模型回答与 gold standard 落入**同一区间**，得 **1 分**；
- 若模型回答所在区间与 gold standard 所在区间**相邻**，得 **σ 分**（σ ≈ 0.6827）；
- 若模型回答所在区间与 gold standard 所在区间**不相邻**，得 **0 分**。

各区间之间的完整得分关系见下方得分矩阵（行(row)为标准回答，列(column)为模型回答）：

|  | **强反叙实** | **较强反叙实** | **较弱反叙实** | **弱反叙实** | **非叙实** | **弱正叙实** | **较弱正叙实** | **较强正叙实** | **强正叙实** |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **强反叙实** | 1 | σ | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| **较强反叙实** | σ | 1 | σ | 0 | 0 | 0 | 0 | 0 | 0 |
| **较弱反叙实** | 0 | σ | 1 | σ | 0 | 0 | 0 | 0 | 0 |
| **弱反叙实** | 0 | 0 | σ | 1 | σ | 0 | 0 | 0 | 0 |
| **非叙实** | 0 | 0 | 0 | σ | 1 | σ | 0 | 0 | 0 |
| **弱正叙实** | 0 | 0 | 0 | 0 | σ | 1 | σ | 0 | 0 |
| **较弱正叙实** | 0 | 0 | 0 | 0 | 0 | σ | 1 | σ | 0 |
| **较强正叙实** | 0 | 0 | 0 | 0 | 0 | 0 | σ | 1 | σ |
| **强正叙实** | 0 | 0 | 0 | 0 | 0 | 0 | 0 | σ | 1 |
