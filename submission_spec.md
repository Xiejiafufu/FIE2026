...
Each data instance in the submission file **must contain exactly the following three fields**:
  ...
  | factivity   | string        | Factivity judgment       |
  | confidence  | float / null  | Model confidence score   |

## 4.1 Field Count Check
...
-   Must be an integer within the range **[1, 5000]**
-   Each `id` value **must appear exactly once**
-   Must **correspond one-to-one** with the evaluation set
...
## 5.3 confidence Field

### Case 1: "factivity" = "true" or "false"
...
| Error Type                                          | Example                                                    |
|---------------------------------------------------- |------------------------------------------------------------|
| Incorrect data count                                | 4,999 or 5,001 instances                                   |
| Incorrect field count                               | Fewer or more than 3 fields                                |
| Incorrect field name                                | `fact`, `score`                                            |
| id not in ascending order                           | `1,3,2`                                                    |
| Duplicate id                                        | Two records with `"id" = 15`                               |
| confidence out of range                             | `0.40`, `-0.85`                                            |
| confidence with incorrect decimal places            | `0.7563`                                                   |
| uncertain but confidence is not null                | `{"factivity": "uncertain", "confidence": 0.35}`           |
...
|------------------------ |----------------------------------------------|
| `file format error`     | The file is not a valid JSON file            |
| `file encoding error`   | The file encoding is not UTF-8               |
| `object quantity error` | The number of data instances is not 5,000    |
| `field quantity error`  | The number of fields is not 3                |
| `field name error`      | The field names are incorrect                |
| `field value error`     | The field values do not meet the requirements|
